...
from game.pieces import Rook, Bishop, King, Queen, Knight, Pawn, Centaur, Piece
...
class Board:
    ...
    def render_move_dots(self, piece: Piece, screen: pygame.Surface):
        ...
        for move in moves:
            x, y = move[1]*constants.SQUARE_WIDTH, move[0]*constants.SQUARE_WIDTH
            if self[move[0]][move[1]]:
                screen.blit(big_circle, (x, y))
            else:
                screen.blit(small_circle, (x, y))
    
    def render(self, screen: pygame.Surface, bottom_color: str):
        for row in range(8):
            for col in range(8):
